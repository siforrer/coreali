...
# TODO better use a dict        self.config = ["word_size": 4,"address_incr": 4, "testmem_address": 0, "testmem_size" :16*4]
    ...
    def test_single_access(self):
        ...
        self.rio.write_words(cfg.testmem_address,cfg.word_size,cfg.address_incr,[0])
        self.rio.write_words_masked(cfg.testmem_address,cfg.word_size,cfg.address_incr,[test_value],[test_mask])
        assert self.rio.read_words(cfg.testmem_address,cfg.word_size,cfg.address_incr,1)[0] == int("05",8)
    ...
    def _test_performance_of_function(self, name, function, factor, minimum_requirement = 0):     
        ...
        while True:            
            ...
            if elapsed > 0.1:
                break
        print("{:<20}  {:>10} bit/s = {:>10} accesses/s".format(name, int(cfg.word_size*8*factor*iterations/elapsed) , int(factor*iterations/elapsed)))
  ...
  "function": [
      lambda : self.rio.write_words(cfg.testmem_address,cfg.word_size,cfg.address_incr,[0]),
      lambda : self.rio.read_words(cfg.testmem_address,cfg.word_size,cfg.address_incr,1),
      lambda : self.rio.write_words_masked(cfg.testmem_address,cfg.word_size,cfg.address_incr,[1],[1]),
      lambda : self.rio.write_words(cfg.testmem_address,cfg.word_size,cfg.address_incr,write_values),
      lambda : self.rio.read_words(cfg.testmem_address,cfg.word_size,cfg.address_incr,num_words),
      lambda : self.rio.write_words_masked(cfg.testmem_address,cfg.word_size,cfg.address_incr,write_values,write_values),
      ],
  "factor": [1,1,1,num_words,num_words,num_words]
}
    
        for i in range(len(use_cases["name"])):
            self._test_performance_of_function(use_cases["name"][i],use_cases["function"][i],use_cases["factor"][i],minimum_requirement)